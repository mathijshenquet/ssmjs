...
(function() {
  ...
  argMatchers = [
    token(/^0x[0-9a-fA-F]+/, function(match) {
      return parseInt(match[0]);
    }), token(/^0b([01]+)/, function(match) {
      return parseInt(match[1], 2);
    }), token(/^-?[0-9]+(\.[0-9]+)?/, function(match) {
      return +match[0];
    }), token(/^[a-zA-Z][a-zA-Z_\-0-9]*/, function(match) {
      return match[0];
  ...
  matchArg = function(src) {
    var matcher, res, _i, _len;

    for (_i = 0, _len = argMatchers.length; _i < _len; _i++) {
      matcher = argMatchers[_i];
      if ((res = matcher(src))) {
        return res;
      }
    }
  ...
  global.Parser = {
    parse: function(src) {
      var line, lineNr, lines, match, prevlength, res, _ref;
